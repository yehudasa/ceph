...
int RGWReshard::reshard_bucket(Formatter *formatter,
			       int num_shards,
			       rgw_bucket& bucket,
			       RGWBucketInfo& bucket_info,
			       RGWBucketInfo& new_bucket_info,
			       ...
			       bool verbose)
{
  ...
  ldout (store->ctx(), 0) <<  "*** NOTICE: operation will not remove old bucket index objects ***" << dendl;
  ldout (store->ctx(), 0) << "***         these will need to be removed manually             ***" << dendl;
  ldout (store->ctx(), 0) << "old bucket instance id: " << bucket_info.bucket.bucket_id << dendl;
  ldout (store->ctx(), 0) << "new bucket instance id: " << new_bucket_info.bucket.bucket_id << dendl;
  ...
  for (int i = 0; i < num_source_shards; ++i) {
	...
	return -ret;
      ...
      std::list<rgw_cls_bi_entry>::iterator iter;
      for (iter = entries.begin(); iter != entries.end(); ++iter) {
	rgw_cls_bi_entry& entry = *iter;
	if (verbose) {
	  formatter->open_object_section("entry");
	...
	int ret = store->get_target_shard_id(new_bucket_info, obj.get_hash_object(), &target_shard_id);
	if (ret < 0) {
	  ldout (store->ctx(), 0) << "ERROR: get_target_shard_id() returned ret=" << ret << dendl;
	  return ret;
	}
	...
	if (verbose) {
	  ...
	  ldout (store->ctx(), 0) << " " << total_entries << dendl;
	}
      }
    }
  }
  if (verbose) {
    ...
    ldout (store->ctx(), 0) << "total entries: " << total_entries << dendl;
  }

  ret = target_shards_mgr.finish();
  if (ret < 0) {
    ldout (store->ctx(), 0) << "ERROR: failed to reshard" << dendl;
    return EIO;
  }
  ...
  int r = RGWBucketAdminOp::link(store, bucket_op, &err);
  if (r < 0) {
    ldout (store->ctx(), 0) << "failed to link new bucket instance (bucket_id=" << new_bucket_info.bucket.bucket_id << ": " << err << "; " << cpp_strerror(-r) << dendl;
    return -r;
  }
  return 0;
}
