...
void RGWOp_SIP_GetStageStatus::execute() {
  ...
  http_ret = sip->get_start_marker(sid, shard_id, &start_pos.marker, &start_pos.timestamp);
  if (http_ret < 0) {
    ldout(s->cct, 5) << "ERROR: sip->get_start_marker() returned error: ret=" << http_ret << dendl;
    return;
  }

  http_ret = sip->get_cur_state(sid, shard_id, &cur_pos.marker, &cur_pos.timestamp);
  if (http_ret < 0) {
...
void RGWOp_SIP_GetMarkerInfo::execute() {
  ...
  auto marker_handler = store->svc()->sip_marker->get_handler(sip);
  if (!marker_handler) {
  ...
  http_ret = marker_handler->get_info(sid, shard_id, &sinfo);
  if (http_ret < 0) {
    ldout(s->cct, 0) << "ERROR: failed to fetch marker handler stage marker info: " << cpp_strerror(-http_ret) << dendl;
    return;
  }
...
void RGWOp_SIP_RemoveMarkerInfo::execute() {
  ...
  http_ret = marker_handler->remove_target(*opt_target_id, sid, shard_id, &result);
  if (http_ret < 0) {
    ldout(s->cct, 0) << "ERROR: failed to remove target marker info: " << cpp_strerror(-http_ret) << dendl;
    return;
  }
}
