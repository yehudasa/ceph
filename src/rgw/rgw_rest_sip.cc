...
void RGWOp_SIP_GetInfo::execute(optional_yield y) {
  ...
  if (provider) {
    sip = store->ctl()->si.mgr->find_sip(this, *provider, opt_instance);
  } else {
    sip = store->ctl()->si.mgr->find_sip_by_type(this,
                                                 *data_type,
...
void RGWOp_SIP_GetStageStatus::execute(optional_yield y) {
  ...
  auto sip = store->ctl()->si.mgr->find_sip(this, provider, opt_instance);
  if (!sip) {
