...
int RGWSI_Zone::do_start(optional_yield y, const DoutPrefixProvider *dpp)
{
  ...
  if (found_zone &&
    ...
    ret = current_period->find_zone(dpp,
                                    cur_zone_id,
                                    zonegroup,
                                    &found_period_conf,
                                    y);
  ...
  bool init_from_period = !current_period->get_id().empty();
...
int RGWSI_Zone::init_zg_from_period(optional_yield y)
{
  ...
  if (iter != current_period->get_map().zonegroups.end()) {
    ...
    *zonegroup = iter->second;
    int ret = zonegroup->init(cct, sysobj_svc, y, false);
    if (ret < 0) {
  ...
  for (iter = current_period->get_map().zonegroups.begin();
	...
	if (zonegroup->get_id() == zg.get_id()) {
	  ...
	  int ret = fixed_zg.init(cct, sysobj_svc, y);
	  if (ret < 0) {
...
int RGWSI_Zone::init_default_zone(optional_yield y)
{
