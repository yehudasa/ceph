...
int RGWSI_BILog_RADOS::log_trim(const RGWBucketInfo& bucket_info, std::optional<uint64_t> opt_gen, int shard_id, string& start_marker, string& end_marker)
{
  ...
  int r = svc.bi->open_bucket_index(bucket_info, opt_gen, shard_id, &index_pool, &bucket_objs, nullptr);
  if (r < 0) {
    return r;
  }
...
int RGWSI_BILog_RADOS::log_start(const RGWBucketInfo& bucket_info, std::optional<uint64_t> opt_gen, int shard_id)
{
  RGWSI_RADOS::Pool index_pool;
  map<int, string> bucket_objs;
  int r = svc.bi->open_bucket_index(bucket_info, opt_gen, shard_id, &index_pool, &bucket_objs, nullptr);
...
int RGWSI_BILog_RADOS::log_stop(const RGWBucketInfo& bucket_info, std::optional<uint64_t> opt_gen, int shard_id)
{
  RGWSI_RADOS::Pool index_pool;
  map<int, string> bucket_objs;
  int r = svc.bi->open_bucket_index(bucket_info, opt_gen, shard_id, &index_pool, &bucket_objs, nullptr);
...
int RGWSI_BILog_RADOS::log_list(const RGWBucketInfo& bucket_info, std::optional<uint64_t> opt_gen, int shard_id, string& marker, uint32_t max,
				      ...
				      optional_yield y)
{
  ...
  int r = svc.bi->cls_bucket_head(bucket_info, opt_gen, shard_id, &headers, &bucket_instance_ids, null_yield);
  if (r < 0)
    return r;
