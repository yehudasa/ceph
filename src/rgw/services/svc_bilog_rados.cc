...
int RGWSI_BILog_RADOS::log_trim(const RGWBucketInfo& bucket_info, int shard_id, uint64_t gen_id, string& start_marker, string& end_marker)
{
  ...
  int r = svc.bi->open_bucket_index(bucket_info, shard_id, gen_id, &index_pool, &bucket_objs, nullptr);
  if (r < 0) {
    return r;
  }
...
int RGWSI_BILog_RADOS::log_start(const RGWBucketInfo& bucket_info, int shard_id, uint64_t gen_id)
{
  RGWSI_RADOS::Pool index_pool;
  map<int, string> bucket_objs;
  int r = svc.bi->open_bucket_index(bucket_info, shard_id, gen_id, &index_pool, &bucket_objs, nullptr);
...
int RGWSI_BILog_RADOS::log_stop(const RGWBucketInfo& bucket_info, int shard_id, uint64_t gen_id)
{
  RGWSI_RADOS::Pool index_pool;
  map<int, string> bucket_objs;
  int r = svc.bi->open_bucket_index(bucket_info, shard_id, gen_id, &index_pool, &bucket_objs, nullptr);
...
int RGWSI_BILog_RADOS::log_list(const RGWBucketInfo& bucket_info, int shard_id, uint64_t gen_id, string& marker, uint32_t max,
                                std::list<rgw_bi_log_entry>& result, uint64_t *generation, bool *truncated)
{
  ...
  int r = svc.bi->open_bucket_index(bucket_info, shard_id, gen_id, &index_pool, &oids, nullptr);
  if (r < 0)
    return r;
  ...
  auto layout = bucket_info.find_log_layout(gen_id);
				      ...
				      optional_yield y)
{
  ...
  int r = svc.bi->cls_bucket_head(bucket_info, shard_id, gen_id, &headers, &bucket_instance_ids, y);
  if (r < 0)
    return r;
