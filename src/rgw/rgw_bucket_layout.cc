...
void encode(const BucketLayout& l, bufferlist& bl, uint64_t f)
{
  ...
  encode(l.current.index, bl);
  encode(l.target_index, bl);
  encode(l.current.log, bl);
  encode(l.indexes, bl);
  encode(l.logs, bl);
  ENCODE_FINISH(bl);
}
...
void decode(BucketLayout& l, bufferlist::const_iterator& bl)
{
  ...
  if (struct_v < 2) {
    ...
    if (l.current.index.layout.type == BucketIndexType::Normal) {
      const auto& index = l.current.index.layout.normal;
      l.current.log = log_layout_from_index(l.current.index.gen, index);
      l.logs[l.current.log.gen] = l.current.log;
    }
  } else {
    decode(l.current.log, bl);
    decode(l.indexes, bl);
    decode(l.logs, bl);
  }
