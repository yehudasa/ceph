...
int BucketAsyncRefreshHandler::init_fetch()
{
  ...
  r = store->getRados()->get_bucket_stats_async(bucket_info, std::nullopt, RGW_NO_SHARD, this);
  if (r < 0) {
...
int RGWBucketStatsCache::fetch_stats_from_storage(const rgw_user& user, const rgw_bucket& bucket, RGWStorageStats& stats, optional_yield y)
{
  ...
  r = store->getRados()->get_bucket_stats(bucket_info, std::nullopt, RGW_NO_SHARD, &bucket_ver,
