...
int RGWPutObjProcessor_Aio::throttle_data(void *handle, const rgw_raw_obj& obj, bool need_to_wait)
{
                                         ...
                                         bool *is_truncated)
{
  ...
  rgw_obj_key end_marker_obj(params.end_marker.name, params.end_marker.instance, params.ns);
...
int RGWRados::get_obj_state_impl(RGWObjectCtx *rctx, const RGWBucketInfo& bucket_info, const rgw_obj& obj,
                                 RGWObjState **state, bool follow_olh)
{
  ...
  *state = s;
  ...
  int r = RGWRados::raw_obj_stat(raw_obj, &s->size, &s->mtime, &s->epoch, &s->attrset, (s->prefetch_data ? &s->data : NULL), NULL);
  if (r == -ENOENT) {
    s->exists = false;
    s->has_attrs = true;
...
int RGWRados::get_obj_state(RGWObjectCtx *rctx, const RGWBucketInfo& bucket_info, const rgw_obj& obj, RGWObjState **state, bool follow_olh)
{
  int ret;
...
{
  return store->get_obj_state(&ctx, bucket_info, obj, pstate, follow_olh);
}
