				      ...
				      check_filter_t force_check_filter)
{
  ...
  int r = svc.bi_rados->open_bucket_index(bucket_info, gen_id, shard_id,
...
int RGWRados::cls_bucket_list_unordered(RGWBucketInfo& bucket_info,
  ...
  *is_truncated = false;
  ...
  map<int, string> oids;
  int r = svc.bi_rados->open_bucket_index(bucket_info, gen_id, shard_id, &index_pool, &oids, nullptr);
  if (r < 0)
    return r;
