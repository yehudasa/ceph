...
public:
  ...
  int process() override {
    ...
    if (store->svc()->zone->sync_module_exports_data()) {
      ...
      data->call(create_data_log_trim_cr(this, static_cast<rgw::sal::RGWRadosStore*>(store), &http,
                               ...
                               const FetchRemoteObjParams& params)
{
  ...
  AtomicObjectProcessor processor(&aio, this->store, dest_bucket, nullptr, user_id,
                                  obj_ctx, std::move(dest_obj->clone()), params.olh_epoch, tag, params.dpp, null_yield);
