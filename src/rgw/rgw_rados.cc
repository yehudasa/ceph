...
int RGWMetaNotifier::process()
{
  ...
  notify_mgr.notify_all(store->ctl.remote->get_zone_meta_notify_to_map(), shards);
...
int RGWDataNotifier::process()
{
  ...
  notify_mgr.notify_all(store->ctl.remote->get_zone_data_notify_to_map(), shards);
...
int RGWRados::init_complete()
{
  ...
  if (run_sync_thread) {
    ...
    for (auto entry : svc.zone->get_data_sync_source_zones()) {
      ...
      ldout(cct, 5) << "starting data sync thread for zone " << zname << dendl;
...
int RGWRados::initialize()
{
  ...
  ctl.si.mgr->register_sip("bucket.inc", std::make_shared<RGWSIPGen_BucketInc>(cct, store, ctl.bucket));
                               ...
                               const FetchRemoteObjParams& params)
{
  ...
  auto dest_obj = _dest_obj->get_obj();
  ...
  AtomicObjectProcessor processor(&aio, this->store, dest_bucket, nullptr, user_id,
                                  obj_ctx, dest_obj, params.olh_epoch, tag, params.dpp, null_yield);
  ...
  RGWRadosPutObj cb(cct, plugin, compressor, &processor,
  ...
  if (params.copy_if_newer) {
    ...
    ret = get_obj_state(&obj_ctx, dest_bucket->get_info(), dest_obj, &dest_state, false, null_yield);
    if (ret < 0)
      goto set_err_state;
  ...
  ret = conn->get_obj(user_id, params.info, src_obj, pmod, params.unmod_ptr,
                      dest_mtime_weight.zone_short_id, dest_mtime_weight.pg_ver,
                      prepend_meta, get_op, rgwx_stat,
                      sync_manifest, skip_decrypt,
                      true,
...
set_err_state:
  if (params.copy_if_newer && ret == -ERR_NOT_MODIFIED) {
    ...
    if (params.olh_epoch && *params.olh_epoch > 0) {
      ...
      ret = set_olh(obj_ctx, dest_bucket->get_info(), dest_obj, false, nullptr,
