...
class RGWRemoteAdminOp {
 ...
 public:
  ...
  /* explicit conn */
  int create_user(const DoutPrefixProvider *dpp, RGWRESTConn *conn, const CreateUserParams& params, optional_yield y);

  /* implicit conn, using conn to primary */
  int create_user(const DoutPrefixProvider *dpp, const CreateUserParams& params, optional_yield y);
};
