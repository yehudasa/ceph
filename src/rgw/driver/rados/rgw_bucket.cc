...
static int bucket_stats(rgw::sal::Driver* driver,
  ...
  ret = bucket->read_stats(dpp, index, snap_range, RGW_NO_SHARD, &bucket_ver, &master_ver, stats, &max_marker);
  if (ret < 0) {
				  ...
				  bool warnings_only)
{
  ...
  for (const auto& user_id : user_ids) {
	...
	ret = bucket->read_stats(dpp, index, rgw_bucket_snap_range(), RGW_NO_SHARD, &bucket_ver, &master_ver, stats, nullptr);
