...
void RGWBucketInfo::dump(Formatter *f) const
{
  ...
  encode_json("num_shards", layout.current.index.layout.normal.num_shards, f);
  encode_json("bi_shard_hash_type", (uint32_t)layout.current.index.layout.normal.hash_type, f);
  ...
  encode_json("index_type", (uint32_t)layout.current.index.layout.type, f);
...
void RGWBucketInfo::decode_json(JSONObj *obj) {
  ...
  JSONDecoder::decode_json("num_shards", layout.current.index.layout.normal.num_shards, obj);
  uint32_t hash_type;
  JSONDecoder::decode_json("bi_shard_hash_type", hash_type, obj);
  layout.current.index.layout.normal.hash_type = static_cast<rgw::BucketHashType>(hash_type);
  ...
  layout.current.index.layout.type = (rgw::BucketIndexType)it;
...
void rgw::bucket_index_layout::dump(Formatter *f) const
{
  ...
  if (type == BucketIndexType::Normal) {
...
void rgw::bucket_index_log_layout::dump(Formatter *f) const
{
  encode_json("gen", gen, f);
  encode_json("layout", layout, f);
}
...
void rgw::BucketLayout::dump(Formatter *f) const
{
  ...
  encode_json("current_index", current.index, f);
  encode_json("current_log", current.log, f);
  encode_json("target_index", target_index, f);
  encode_json("indexes", indexes, f);
  encode_json("logs", logs, f);
}
