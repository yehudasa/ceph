...
int send_remote(const DoutPrefixProvider *dpp,
                RGWRESTConn* conn,
                const string& method,
                const string& path,
...
#define MAX_REST_RESPONSE (128 * 1024)
  int r = conn->forward(dpp, user, info, nullptr, MAX_REST_RESPONSE, nullptr, outbl, y);
  if (r < 0) {
    return r;
  }
...
int RGWRemoteAdminOp::create_user(const DoutPrefixProvider *dpp,
                                  RGWRESTConn *conn,
                                  const CreateUserParams& params,
                                  optional_yield y)
{
  if (!conn) {
    ldpp_dout(dpp, 1) << __func__ << "(): connection is not initialized" << dendl;
    return -EINVAL;
  }
  ...
  int r = send_remote(dpp,
                      ...
                      y);
  if (r < 0) {
    ldpp_dout(dpp, 5) << "remote request failed: r=" << r << dendl;
    return r;
  }
  return 0;
}
                                  ...
                                  optional_yield y)
{
  if (svc.zone->is_meta_master()) {
    ...
    ldpp_dout(dpp, 1) << "Zone is meta primary, no remote connection to ourselves to perform operation" << dendl;
    return -EINVAL;
  }

  return create_user(dpp, svc.zone->get_master_conn(), params, y);
}
