...
int source_bilog_markers(RGWRemoteCtl *remote_ctl,
                         const rgw_sync_bucket_pipe& pipe,
                         ...
                         optional_yield y)
{
  ...
  return rgw_read_remote_bilog_info(conn->data, *pipe.source.bucket,
                                    remote_markers, y);
}
                               ...
                               ceph::coarse_mono_time timeout_at)
{
  ...
  for (const auto& [source_zone_id, pipe] : policy.get_all_sources()) {
    ...
    spawn::spawn(ioctx, [&] (spawn::yield_context yield) {
      ...
      int r = source_bilog_markers(store->ctl()->remote, entry.pipe,
