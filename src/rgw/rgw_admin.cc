...
static void get_data_sync_status(const rgw_zone_id& source_zone, list<string>& status, int tab)
{
  ...
  if (!static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->zone->find_data_provider(source_zone, &sz)) {
...
static void sync_status(Formatter *formatter)
{
  ...
  auto& source_zones = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->zone->get_data_sync_source_zones();
  ...
  for (auto& entry : source_zones) {
    ...
    RGWDataProvider *sz;
    if (static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->zone->find_data_provider(source_id, &sz)) {
      s += string(" (") + sz->name + ")";
    }
...
static rgw_zone_id resolve_zone_id(const string& s)
{
  ...
  if (static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->zone->find_data_provider(s, &zone)) {
...
class JSONFormatter_PrettyZone : public JSONFormatter {
  class Handler : public JSONEncodeFilter::Handler<rgw_zone_id> {
    void encode_json(const char *name, const void *pval, ceph::Formatter *f) const override {
      ...
      string zone_name;
      RGWDataProvider *dp;
      if (static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->zone->find_data_provider(zone_id, &dp)) {
        zone_name = dp->name;
      } else {
...
#define MAX_LC_LIST_ENTRIES 100
 ...
 if (opt_cmd == OPT::BUCKET_SYNC_RUN) {
  ...
  if (opt_cmd == OPT::DATALOG_AUTOTRIM) {
    ...
    ret = crs.run(create_admin_data_log_trim_cr(dpp(), static_cast<rgw::sal::RGWRadosStore*>(store), &http, num_shards));
    if (ret < 0) {
