...
static void get_data_sync_status(const rgw_zone_id& source_zone, list<string>& status, int tab)
{
  ...
  if (!store->svc()->zone->find_data_provider(source_zone, &sz)) {
...
static void sync_status(Formatter *formatter)
{
  ...
  for (auto& entry : source_zones) {
    ...
    RGWDataProvider *sz;
    if (store->svc()->zone->find_data_provider(source_id, &sz)) {
      s += string(" (") + sz->name + ")";
    }
...
static rgw_zone_id resolve_zone_id(const string& s)
{
  ...
  if (store->svc()->zone->find_data_provider(s, &zone)) {
...
class JSONFormatter_PrettyZone : public JSONFormatter {
  class Handler : public JSONEncodeFilter::Handler<rgw_zone_id> {
    void encode_json(const char *name, const void *pval, ceph::Formatter *f) const override {
      ...
      string zone_name;
      RGWDataProvider *dp;
      if (store->svc()->zone->find_data_provider(zone_id, &dp)) {
        zone_name = dp->name;
      } else {
...
#define MAX_LC_LIST_ENTRIES 100
 ...
 if (opt_cmd == OPT::SI_PROVIDER_LIST) {
   SIPCRMgr sip_cr_mgr(cct.get(), store->ctl()->remote, store->getRados()->get_cr_registry());
...
#define MAX_FETCH_CHUNK 1000
 ...
 if (opt_cmd == OPT::SI_PROVIDER_MARKER_TARGET_SET ||
   ...
   auto marker_handler = store->svc()->sip_marker->get_handler(provider);
   if (!marker_handler) {
