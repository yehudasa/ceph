...
int rgw_remove_bucket_bypass_gc(RGWRados *store, rgw_bucket& bucket, int concurrent_max)
{
  int ret;
  map<RGWObjCategory, RGWStorageStats> stats;
  std::vector<RGWObjEnt> objs;
    ...
    for (it = objs.begin(); it != objs.end(); ++it) {
      ...
      if (astate->has_manifest) {
        ...
        for (; miter != manifest.obj_end() && max_aio--; ++miter) {
          ...
          store->delete_obj_aio(last_obj, bucket, info, astate, handles);
        } // for all shadow objs

        store->delete_obj_aio(head_obj, bucket, info, astate, handles);
      }
...
int RGWBucket::remove(RGWBucketAdminOpState& op_state, bool bypass_gc, std::string *err_msg)
{
  ...
  if (bypass_gc) {
    if (delete_children) {
      ret = rgw_remove_bucket_bypass_gc(store, bucket, op_state.get_max_aio());
    } else {
...
int RGWBucketAdminOp::remove_bucket(RGWRados *store, RGWBucketAdminOpState& op_state, bool bypass_gc)
{
  RGWBucket bucket;
  ...
  return bucket.remove(op_state, bypass_gc);
}
