...
class RGWRunBucketSourcesSyncCoroutine : public RGWCoroutine {
  RGWDataSyncEnv *sync_env;
  rgw_bucket_shard bs;
...
public:
  RGWRunBucketSyncSourcesCoroutine(RGWDataSyncEnv *_sync_env, const rgw_bucket_shard& _bs, const RGWSyncTraceNodeRef& _tn_parent)
    : RGWCoroutine(_sync_env->cct), sync_env(_sync_env), bs(_bs),
      status_oid(RGWBucketSyncStatusManager::source_status_oid(sync_env->source_zone, bs)),
...
int RGWRunBucketSourcesSyncCoroutine::operate()
{
  reenter(this) {
    yield {
      set_status("acquiring sync lock");
    ...
    tn->log(10, "took lease");
    yield call(new RGWReadBucketSourcesInfoCoroutine(sync_env, bs.bucket, &info));
    if (retcode < 0 && retcode != -ENOENT) {
