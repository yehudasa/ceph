...
public:
  RGWRunBucketSyncCoroutine(RGWBucketSyncCtx *_bsc,
    ...
    : RGWCoroutine(_bsc->sc->cct), bsc(_bsc), sync_env(_bsc->sc->env),
      ...
      sync_pair(_bsc->si->get_sync_pair()), progress(progress),
...
public:
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      /* fetch current position in logs */
      yield call(bsc->si->get_inc_pos_cr(&info));
      if (retcode < 0 && retcode != -ENOENT) {
      ...
      yield {
        ...
        if (write_status) {
          call(bsc->si->write_status_cr(dpp, status));
        } else {
          call(bsc->si->remove_status_cr());
        }
      }
      if (info.syncstopped) {
        retcode = -ENOENT;
                                               ...
                                               const rgw_bucket& dest_bucket) : dpp(_dpp), sync_env(_sync_env)
{
  ...
  for (int i = 0; i < num_shards; ++i) {
    auto bsi = new RGWBucketSyncInfoCRHandler_Legacy(&sc, sync_pairs[i]);
    bsis[i].reset(bsi);
    bscs[i].init(&sc, bsi);
  }
}
...
RGWCoroutine *RGWRemoteBucketManager::read_sync_status_cr(int num, rgw_bucket_shard_sync_info *sync_status)
{
  ...
  return bscs[num].si->read_status_cr(sync_status);
}
...
public:
  ...
  RGWCoroutine* store_marker(const DoutPrefixProvider *dpp, const rgw_obj_key& new_marker, uint64_t index_pos, const real_time& timestamp) override {
    ...
    auto cr = bsc->si->write_status_cr(dpp, status);
...
class RGWWriteBucketShardIncSyncStatus : public RGWCoroutine {
 ...
 public:
  ...
  int operate(const DoutPrefixProvider *dpp) {
    reenter(this) {
      yield call(bsc->si->write_status_cr(dpp, status));
      if (retcode < 0) {
        return set_cr_error(retcode);
      }
      if (stable_timestamp) {
...
#warning move prefix handler into bsc->si
      ...
      yield call(bsc->si->fetch_full_cr(list_marker, &list_result));
      if (retcode < 0 && retcode != -ENOENT) {
    ...
    if (sync_status == 0) {
      yield {
        ...
        call(bsc->si->write_status_cr(dpp, sync_info));
      }
    } else {
...
int RGWBucketShardIncrementalSyncCR::operate(const DoutPrefixProvider *dpp)
{
  ...
  reenter(this) {
    do {
      ...
      yield call(bsc->si->fetch_inc_cr(sync_info.inc_marker->position,
                                       &list_result));
      if (retcode < 0 && retcode != -ENOENT) {
        /* wait for all operations to complete */
        drain_all();
      ...
      for (; entries_iter != entries_end; ++entries_iter) {
        ...
        if (e.op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP) {
          ldout(dpp, 20) << "syncstop on " << e.timestamp << dendl;
          syncstopped = true;
      ...
      for (; entries_iter != entries_end; ++entries_iter) {
        ...
        if (entry->op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP || entry->op == RGWModifyOp::CLS_RGW_OP_RESYNC) {
          ldout(dpp, 20) << "detected syncstop or resync on " << entries_iter->timestamp << ", skipping entry" << dendl;
  ...
  reenter(this) {
    ...
    for (siter = pipes.begin(); siter != pipes.end(); ++siter) {
      ...
      for (; num_shards > 0; --num_shards, ++cur_shard) {
        ...
        cur_stack->bsc.init(sc, cur_stack->bsi.get());
...
{
  reenter(this) {
    yield call(bsc->si->read_status_cr(&sync_status));
    if (retcode < 0 && retcode != -ENOENT) {
      tn->log(0, "ERROR: failed to read sync status for bucket");
      drain_all();
      return set_cr_error(retcode);
