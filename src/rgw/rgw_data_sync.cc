...
class RGWDataSyncInfoCRHandler_SIP : public RGWDataSyncInfoCRHandler {
  ...
  class FetchCR : public RGWCoroutine {
    RGWDataSyncInfoCRHandler_SIP *handler;
  ...
  public:
    FetchCR(RGWDataSyncInfoCRHandler_SIP *_handler,
            RGWDataSyncCtx *_sc,
...
#warning FIXME sid
    auto& sid = full_stages.front();
    return new FetchCR(this, sc, sip_full.get(), sid, 0 /* shard id */, marker, result);
  }
...
#warning FIXME
  ...
  RGWCoroutine *fetch_inc_cr(int shard_id,
                             ...
                             sip_data_list_result *result) {
#warning FIXME sid
    auto& sid = inc_stages.front();
    return new FetchCR(this, sc, sip_inc.get(), sid, shard_id, marker, result);
  }
};
    ...
    reenter(this) {
      ...
      do {
        ...
        for (iter = result.entries.begin(); iter != result.entries.end(); ++iter) {
          ldout(sync_env->cct, 20) << "fetch full: bucket=" << iter->bucket << dendl;
...
public:
  RGWRunBucketSyncCoroutine(RGWBucketSyncCtx *_bsc,
    ...
    : RGWCoroutine(_bsc->sc->cct), bsc(_bsc), sync_env(_bsc->sc->env),
      ...
      sync_pair(_bsc->hsi->get_sync_pair()), progress(progress),
  ...
  public:
    ...
    int operate() {
      reenter(this) {
        ...
        if (retcode < 0) {
          ldout(sync_env->cct, 0) << "ERROR: failed to call fetch bucket shard info obj=" << status_obj << " ret=" << retcode << dendl;
          return set_cr_error(retcode);
        }
...
public:
  ...
  int operate() override {
    reenter(this) {
      /* fetch current position in logs */
      yield call(bsc->hsi->get_inc_pos_cr(&info));
      if (retcode < 0 && retcode != -ENOENT) {
      ...
      yield {
        ...
        if (write_status) {
          call(bsc->hst->write_status_cr(status));
        } else {
                                               ...
                                               const rgw_bucket& dest_bucket) : dpp(_dpp), sync_env(_sync_env)
{
  ...
  for (int i = 0; i < num_shards; ++i) {
    auto& handlers = bh[i];
    handlers.info.reset(new RGWBucketSyncInfoCRHandler_Legacy(&sc, sync_pairs[i]));
    handlers.status.reset(new RGWBucketSyncStatusCRHandler_Legacy(&sc, sync_pairs[i]));
    bscs[i].init(&sc, handlers.info.get(), handlers.status.get());
  }
}
...
public:
  ...
  RGWCoroutine *store_marker(const rgw_obj_key& new_marker, uint64_t index_pos, const real_time& timestamp) override {
    ...
    auto cr = bsc->hst->write_status_cr(status);
...
class RGWWriteBucketShardIncSyncStatus : public RGWCoroutine {
 ...
 public:
  ...
  int operate() {
    reenter(this) {
      yield call(bsc->hst->write_status_cr(status));
      if (retcode < 0) {
        return set_cr_error(retcode);
      }
      if (stable_timestamp) {
...
#warning move prefix handler into bsc->hsi
      ...
      yield call(bsc->hsi->fetch_full_cr(list_marker, &list_result));
      if (retcode < 0 && retcode != -ENOENT) {
    ...
    if (sync_status == 0) {
      yield {
        ...
        call(bsc->hst->write_status_cr(sync_info));
      }
    } else {
...
int RGWBucketShardIncrementalSyncCR::operate()
{
  ...
  reenter(this) {
    do {
      ...
      yield call(bsc->hsi->fetch_inc_cr(sync_info.inc_marker->position,
                                        &list_result));
      if (retcode < 0 && retcode != -ENOENT) {
        /* wait for all operations to complete */
        drain_all();
      ...
      for (; entries_iter != entries_end; ++entries_iter) {
        ...
        if (e.op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP) {
          ldout(bsc->cct, 20) << "syncstop on " << e.timestamp << dendl;
          syncstopped = true;
      ...
      for (; entries_iter != entries_end; ++entries_iter) {
        ...
        if (entry->op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP || entry->op == RGWModifyOp::CLS_RGW_OP_RESYNC) {
          ldout(bsc->cct, 20) << "detected syncstop or resync on " << entries_iter->timestamp << ", skipping entry" << dendl;
  ...
  reenter(this) {
    ...
    for (siter = pipes.begin(); siter != pipes.end(); ++siter) {
      ...
      for (; num_shards > 0; --num_shards, ++cur_shard) {
        ...
        cur_stack->bsi.reset(new RGWBucketSyncInfoCRHandler_Legacy(sc, sync_pair));
        cur_stack->bst.reset(new RGWBucketSyncStatusCRHandler_Legacy(sc, sync_pair));
        cur_stack->bsc.init(sc, cur_stack->bsi.get(), cur_stack->bst.get());
