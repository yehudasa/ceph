...
{
  return run(new RGWReadRemoteDataLogInfoCR(&sc, sc.dsi.inc->num_shards(), shards_info));
}
  ...
  int operate() override {
    reenter(this) {
        ...
        for (iter = result.entries.begin(); iter != result.entries.end(); ++iter) {
          ...
          if (iter->num_shards > 0) {
            for (i = 0; i < iter->num_shards; i++) {
              ...
              s = iter->key + buf;
              yield entries_index->append(s, sync_env->svc->datalog_rados->calc_shard_id(iter->bucket, i, num_shards));
            }
          } else {
            yield entries_index->append(iter->key, sync_env->svc->datalog_rados->calc_shard_id(iter->bucket, -1, num_shards));
          }
        }
      ...
      if (!failed) {
        ...
        yield call(sync_env->error_logger->log_error_cr(sc->conns.data->get_remote_id(), "data.init", "",
...
public:
  ...
  RGWCoroutine *store_marker(const string& new_marker, const string& key, uint64_t index_pos, const real_time& timestamp) override {
...
#warning FIXME
#if 0
  virtual int validate_sync_marker(rgw_bucket_index_marker_info& marker) const = 0;
#endif
};
    ...
    reenter(this) {
      ...
      if (sync_status < 0) {
        ...
        if (sync_status != -EBUSY && sync_status != -EAGAIN) {
          yield call(sync_env->error_logger->log_error_cr(sc->conns.data->get_remote_id(), "data", complete->key,
...
public:
  ...
  RGWCoroutine *store_marker(const std::string& new_marker, const rgw_obj_key& key, uint64_t index_pos, const real_time& timestamp) override {
...
public:
  ...
  RGWCoroutine *store_marker(const string& new_marker, const rgw_obj_key& key, uint64_t index_pos, const real_time& timestamp) override {
  ...
  int operate() override {
    reenter(this) {
      ...
      if (!error_ss.str().empty()) {
        yield call(sync_env->error_logger->log_error_cr(sc->conns.data->get_remote_id(), "data", error_ss.str(), -retcode, string("failed to sync object") + cpp_strerror(-sync_status)));
      }
done:
      if (sync_status == 0) {
        /* update marker */
    ...
    do {
      ...
      for (; entries_iter != entries_end; ++entries_iter) {
        ...
        if (e.op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP) {
          ldout(bsc->cct, 20) << "syncstop on " << e.mtime << dendl;
          syncstopped = true;
      ...
      for (; entries_iter != entries_end; ++entries_iter) {
        ...
        if (entry->op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP || entry->op == RGWModifyOp::CLS_RGW_OP_RESYNC) {
          ldout(bsc->cct, 20) << "detected syncstop or resync on " << entries_iter->mtime << ", skipping entry" << dendl;
                           ...
                           std::vector<rgw_bucket_index_marker_info> *opt_source_markers)
{
  ...
  ret = crs.run(wrapper_core_inc->init_cr("bucket.inc", "legacy/bucket.inc", tn));
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: " << __func__ << "(): failed to initialize SIP core for incremental bucket sync: ret=" << ret << dendl;
    return ret;
  }

  ret = crs.run(new RGWCollectBucketSyncStatusCR(store, &sc,
                                                 source_bucket,
  ...
  return crs.run(new RGWCollectBucketSourceShardStateCR(store, &sc,
                                                        wrapper_core_inc,
                                                        opt_source_markers));
}
