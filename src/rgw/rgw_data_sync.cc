...
public:
  RGWBucketShardIncrementalSyncCR(RGWDataSyncEnv *_sync_env,
    ...
    : RGWCoroutine(_sync_env->cct), sync_env(_sync_env), bs(bs),
      ...
      marker_tracker(sync_env, status_oid, inc_marker), status_oid(status_oid) , zone_id(_sync_env->store->svc.zone->get_zone().id){
  ...
  reenter(this) {
    ...
    if (syncstopped) {
      ...
      yield {
        ...
        RGWRados *store = sync_env->store;
        call(new RGWRadosRemoveCR(store, rgw_raw_obj{store->svc.zone->get_zone_params().log_pool, oid}));
      }
      lease_cr->abort();
      return set_cr_done();
    }
  ...
  reenter(this) {
    ...
    if (retcode == -ENOENT) {
      ...
      yield {
        ...
        meta_sync_env.init(cct, sync_env->store, sync_env->store->svc.zone->get_master_conn(), sync_env->async_rados,
