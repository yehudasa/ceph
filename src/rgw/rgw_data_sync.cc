...
public:
  ...
  virtual RGWCoroutine *get_inc_pos_cr(rgw_bucket_index_marker_info *info) = 0;
  ...
  virtual RGWCoroutine *fetch_full_cr(const rgw_obj_key& list_marker,
                                      bucket_list_result *list_result) = 0;
  virtual RGWCoroutine *fetch_inc_cr(const string& marker,
                                     list<rgw_bi_log_entry> *result) = 0;
...
public:
  RGWRunBucketSyncCoroutine(RGWDataSyncCtx *_sc,
                            ...
                            ceph::real_time* progress)
    : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
      lease_cr(std::move(lease_cr)), bsi(_bsi),
      sync_pair(_bsi->get_sync_pair()), progress(progress),
...
class RGWBucketSyncInfoCRHandler_Legacy : public RGWBucketPipeSyncInfoCRHandler {
  string sync_status_oid;
...
public:
  ...
  RGWBucketSyncInfoCRHandler_Legacy(RGWDataSyncCtx *_sc,
  ...
  RGWCoroutine *get_inc_pos_cr(rgw_bucket_index_marker_info *info) override {
  ...
  RGWCoroutine *fetch_full_cr(const rgw_obj_key& list_marker,
                              bucket_list_result *list_result) override;
  RGWCoroutine *fetch_inc_cr(const string& marker,
                             list<rgw_bi_log_entry> *result) override;
};
...
public:
  ...
  int operate(const DoutPrefixProvider *dpp) override {
    reenter(this) {
      /* fetch current position in logs */
      yield call(bsi->get_inc_pos_cr(&info));
      if (retcode < 0 && retcode != -ENOENT) {
      ...
      yield {
        ...
        if (write_status) {
          call(bsi->write_status_cr(dpp, status));
        } else {
          call(bsi->remove_status_cr());
        }
      }
      if (info.syncstopped) {
        retcode = -ENOENT;
                                               ...
                                               const rgw_bucket& dest_bucket) : dpp(_dpp), sync_env(_sync_env)
{
  ...
  bsis.resize(num_shards);

  for (int i = 0; i < num_shards; ++i) {
    bsis[i].reset(new RGWBucketSyncInfoCRHandler_Legacy(&sc, sync_pairs[i]));
  }
}
...
RGWCoroutine *RGWRemoteBucketManager::init_sync_status_cr(int num)
{
  ...
  return new RGWInitBucketShardSyncStatusCoroutine(&sc, bsis[num], init_status);
}
...
RGWCoroutine *RGWRemoteBucketManager::read_sync_status_cr(int num, rgw_bucket_shard_sync_info *sync_status)
{
  ...
  return bsis[num]->read_status_cr(sync_status);
}
...
public:
  RGWListBucketShardCR(RGWDataSyncCtx *_sc, const rgw_bucket_shard& bs,
                       const rgw_obj_key& _marker_position, bucket_list_result *_result)
...
RGWCoroutine *RGWBucketSyncInfoCRHandler_Legacy::fetch_full_cr(const rgw_obj_key& list_marker,
                                                               bucket_list_result *list_result)
{
  return new RGWListBucketShardCR(sc, sync_pair.source_bs, list_marker, list_result);
}
...
RGWCoroutine *RGWBucketSyncInfoCRHandler_Legacy::fetch_inc_cr(const string& marker,
                                                              list<rgw_bi_log_entry> *result)
{
...
public:
  ...
  RGWCoroutine* store_marker(const DoutPrefixProvider *dpp, const rgw_obj_key& new_marker, uint64_t index_pos, const real_time& timestamp) override {
    ...
    auto cr = bsi->write_status_cr(dpp, status);
...
class RGWWriteBucketShardIncSyncStatus : public RGWCoroutine {
 ...
 public:
  ...
  int operate(const DoutPrefixProvider *dpp) {
    reenter(this) {
      yield call(bsi->write_status_cr(dpp, status));
      if (retcode < 0) {
        return set_cr_error(retcode);
      }
      if (stable_timestamp) {
...
#warning move prefix handler into bsi
      ...
      yield call(bsi->fetch_full_cr(list_marker, &list_result));
      if (retcode < 0 && retcode != -ENOENT) {
    ...
    if (sync_status == 0) {
      yield {
        ...
        call(bsi->write_status_cr(dpp, sync_info));
      }
    } else {
...
int RGWBucketShardIncrementalSyncCR::operate(const DoutPrefixProvider *dpp)
{
  ...
  reenter(this) {
    do {
      ...
      yield call(bsi->fetch_inc_cr(sync_info.inc_marker->position,
                                   &list_result));
      if (retcode < 0 && retcode != -ENOENT) {
        /* wait for all operations to complete */
        drain_all();
  ...
  reenter(this) {
    ...
    for (siter = pipes.begin(); siter != pipes.end(); ++siter) {
      ...
      for (; num_shards > 0; --num_shards, ++cur_shard) {
        ...
        bsi.reset(new RGWBucketSyncInfoCRHandler_Legacy(sc, sync_pair));

        yield_spawn_window(new RGWRunBucketSyncCoroutine(sc, lease_cr, bsi, tn,
                                                         cur_progress),
...
{
  reenter(this) {
    yield call(bsi->read_status_cr(&sync_status));
    if (retcode < 0 && retcode != -ENOENT) {
      tn->log(0, "ERROR: failed to read sync status for bucket");
      drain_all();
      return set_cr_error(retcode);
    ...
    do {
      if (sync_status.state == rgw_bucket_shard_sync_info::StateInit ||
        ...
        yield call(new RGWInitBucketShardSyncStatusCoroutine(sc, bsi, sync_status));
        if (retcode == -ENOENT) {
...
RGWCoroutine *RGWRemoteBucketManager::run_sync_cr(int num)
{
  ...
  return new RGWRunBucketSyncCoroutine(&sc, nullptr, bsis[num], sync_env->sync_tracer->root_node, nullptr);
}
...
class RGWCollectBucketSyncStatusCR : public RGWShardCollectCR {
 ...
 public:
  ...
  bool spawn_next() override {
    ...
    bsi.reset(new RGWBucketSyncInfoCRHandler_Legacy(sc, sync_pair));
    spawn(bsi->read_status_cr(&*i), false);
    ++i;
    ++source_bs.shard_id;
