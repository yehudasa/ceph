...
int RocksDBStore::submit_common(rocksdb::WriteOptions& woptions, KeyValueDB::Transaction t) 
{
  ...
  *_dout << " Rocksdb transaction: " << bat_txc.seen.str() << dendl;
  ...
  if (cct->_conf->rocksdb_perf) {
    ...
    // DO additions
    logger->tinc(l_rocksdb_block_read_time, block_read_time);
    logger->tinc(l_rocksdb_block_checksum_time, block_checksum_time);
    logger->tinc(l_rocksdb_block_decompress_time, block_decompress_time);
    logger->tinc(l_rocksdb_get_snapshot_time, get_snapshot_time);
    logger->tinc(l_rocksdb_get_from_memtable_time, get_from_memtable_time);
    logger->tinc(l_rocksdb_get_post_process_time, get_post_process_time);
    logger->tinc(l_rocksdb_get_from_output_files_time, get_from_output_files_time);
    logger->tinc(l_rocksdb_seek_on_memtable_time, seek_on_memtable_time);
    logger->tinc(l_rocksdb_seek_child_seek_time, seek_child_seek_time);
    logger->tinc(l_rocksdb_seek_min_heap_time, seek_min_heap_time);
    logger->tinc(l_rocksdb_seek_max_heap_time, seek_max_heap_time);
    logger->tinc(l_rocksdb_seek_internal_seek_time, seek_internal_seek_time);
    logger->tinc(l_rocksdb_find_next_user_entry_time, find_next_user_entry_time);
    logger->tinc(l_rocksdb_write_scheduling_flushes_compactions_time, write_scheduling_flushes_compactions_time);
    logger->tinc(l_rocksdb_write_thread_wait_nanos, write_thread_wait_nanos);
    logger->tinc(l_rocksdb_db_mutex_lock_nanos, db_mutex_lock_nanos);
    logger->tinc(l_rocksdb_db_condition_wait_nanos, db_condition_wait_nanos);
    logger->tinc(l_rocksdb_merge_operator_time_nanos, merge_operator_time_nanos);
    logger->tinc(l_rocksdb_read_index_block_nanos, read_index_block_nanos);
    logger->tinc(l_rocksdb_read_filter_block_nanos, read_filter_block_nanos);
    logger->tinc(l_rocksdb_new_table_block_iter_nanos, new_table_block_iter_nanos);
    logger->tinc(l_rocksdb_new_table_iterator_nanos, new_table_iterator_nanos);
    logger->tinc(l_rocksdb_block_seek_nanos, block_seek_nanos);
    logger->tinc(l_rocksdb_find_table_nanos, find_table_nanos);
    logger->tinc(l_rocksdb_key_lock_wait_time, key_lock_wait_time);
