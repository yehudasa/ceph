                          ...
                          bool create_if_not_found)
{
  ...
  std::unique_lock wl(lock);
  int r = _find(id, kh); /* retry, might have raced */
  if (!create_if_not_found ||
      r != -ENOENT) {
    return r;
  }

  kh = std::make_shared<FSCryptKeyHandler>(++epoch, nullptr);
                                  ...
                                  std::function<FSCryptDenc *()> gen_denc)
{
  ...
  if (kv) {
    *kv = make_shared<FSCryptKeyValidator>(master_kh, master_kh->get_epoch());
  }
