...
ldout(cct, 0) << __FILE__ << ":" << __LINE__ << ":" << __func__ << dendl;
  if (fscrypt_denc) {
    string dname;
...
ldout(cct, 0) << __FILE__ << ":" << __LINE__ << ": CCC bl=" << fscrypt_hex_str(bl->c_str(), bl->length()) << dendl;
  ...
  if(f->readahead.get_min_readahead_size() > 0) {
    pair<uint64_t, uint64_t> readahead_extent = f->readahead.update(off, len, effective_size);
...
ldout(cct, 0) << __FILE__ << ":" << __LINE__ << ":" << " de->alternate_name is empty()" << dendl;
    req->set_alternate_name(std::move(alternate_name));
  } else {
ldout(cct, 0) << __FILE__ << ":" << __LINE__ << ":" << " de->alternate_name=" << fscrypt_hex_str(de->alternate_name.c_str(), de->alternate_name.size()) << dendl;
