...
def test_sync_policy_config_bucket():
    """
    test_sync_policy_config_bucket:
        test configuration of all sync commands at bucket level
    """
    zonegroup = realm.master_zonegroup()
    zonegroup_conns = ZonegroupConns(zonegroup)
...
def test_sync_pipe_single_bucket():
    """
    test_sync_pipe_single_bucket:
        symmetrical flow but configure pipe for only bucketA.

        verify that data sync happens for only bucketA but not for other
        buckets.
    """
...
def test_sync_pipe_different_buckets():
    """
    test_sync_pipe_different_buckets:
        directional flow
...
def test_sync_pipe_multiple_buckets_to_single():
    """
    test_sync_pipe_multiple_buckets_to_single:
        directional flow
...
def test_sync_pipe_single_bucket_to_multiple():
    """
    test_sync_pipe_single_bucket_to_multiple:
        directional flow
...
def test_sync_bucket_level():
    """
    test_sync_bucket_level:
        symmetrical flow but do not enable it at zonegroup level
        configure another policy at bucketA level and enable it
    ...
    # bucketB objects should not be synced
    bucket = get_bucket(zcB, bucketB.name)
    check_object_not_exists(bucket, objnames[1])

    remove_sync_policy_group(c1, "sync-bucket", bucketA.name)
    remove_sync_policy_group(c1, "sync-group")
    return

def test_sync_bucket_to_different_bucket():
    """
    test_sync_bucket_to_different_bucket:
        similar to 'test_sync_pipe_different_buckets' but at bucket level

        symmetrical flow but do not enable it at zonegroup level
        configure another policy at bucketA level with pipe set to
        another bucket(bucketB) in target zone.

        verify zoneA bucketA syncs to zoneB BucketB but not bucketA
    """
...
def test_sync_bucket_from_multiple_buckets():
    """
    test_sync_bucket_from_multiple_buckets:
        similar to test_sync_pipe_multiple_buckets_to_single at bucket level

        symmetrical flow but do not enable it at zonegroup level
        configure another policy at bucketA level with pipe configured
...
def test_sync_bucket_to_multiple_buckets():
    """
    test_sync_bucket_to_multiple_buckets:
        similar to test_sync_pipe_single_bucket_to_multiple at bucket level
