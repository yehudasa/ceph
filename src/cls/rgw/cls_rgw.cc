...
int rgw_bucket_complete_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  CLS_LOG(0, "rgw_bucket_complete_op(): remove_objs.size()=%ld\n", op.remove_objs.size());
...
int rgw_dir_suggest_changes(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  while (!in_iter.end()) {
    ...
    CLS_LOG(20, "cur_disk.pending_map.empty()=%d op=%d cur_disk.exists=%d cur_change.pending_map.size()=%ld cur_change.exists=%d\n",
