...
static void encode_snap_index_key(const cls_rgw_obj_key& key, rgw_bucket_snap_id bsnap_id, string *index_key)
{
  *index_key = BI_PREFIX_CHAR;
  index_key->append(bucket_index_prefixes[BI_BUCKET_SNAP_INDEX]);
  char buf[32];
  snprintf(buf, sizeof(buf), "%lld_", (long long)bsnap_id);
...
int rgw_bucket_complete_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  if (op.op == CLS_RGW_OP_CANCEL) {
    ...
    if (entry.meta.bsnap_id != RGW_BUCKET_NO_SNAP) {
      rc = write_snap_entry(hctx, entry, op.key, entry.meta.bsnap_id);
      if (rc < 0) {
        CLS_LOG_BITX(bitx_inst, 1,
  ...
  int find_next_key(cls_rgw_obj_key *next_key, rgw_bucket_snap_id *next_snap_id, bool *found) {
    ...
    *found = (key.name == next_entry.key.name);
    if (*found) {
      ...
      *next_snap_id = next_entry.meta.bsnap_id;
    }
...
static int rgw_bucket_link_olh(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  /* read instance entry */
  BIVerObjEntry obj(hctx, op.key, op.meta.bsnap_id);
  ...
  if (op.key.instance.empty()) {
    BIVerObjEntry other_obj(hctx, op.key, op.meta.bsnap_id);
  ...
  if (!olh.start_modify(op.olh_epoch)) {
    ...
    if (removing) {
      olh.update_log(CLS_RGW_OLH_OP_REMOVE_INSTANCE, op.op_tag, op.key, op.meta.bsnap_id, false, op.olh_epoch);
    }
  ...
  /* update the olh log */
  olh.update_log(CLS_RGW_OLH_OP_LINK_OLH, op.op_tag, op.key, op.meta.bsnap_id, op.delete_marker);
  if (removing) {
    olh.update_log(CLS_RGW_OLH_OP_REMOVE_INSTANCE, op.op_tag, op.key, op.meta.bsnap_id, false);
  }
  ...
  if (op.meta.bsnap_id != RGW_BUCKET_NO_SNAP) {
    rgw_bucket_dir_entry next_entry;
    std::string next_idx;
    bool found;
    ...
    if (found) {
      ...
      if (next_entry.meta.bsnap_id == RGW_BUCKET_NO_SNAP ||
          next_entry.meta.bsnap_id < op.meta.bsnap_id) {
        obj.set_snap_skip(next_entry.meta.bsnap_id, next_idx);
      } else {
