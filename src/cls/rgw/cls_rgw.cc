...
static int get_obj_vals(cls_method_context_t hctx, const string& start, const string& filter_prefix,
                        int num_entries, map<string, bufferlist> *pkeys, bool *ptruncated)
{
  int ret = cls_cxx_map_get_vals(hctx, start, filter_prefix, num_entries, pkeys, ptruncated);
  if (ret < 0)
    return ret;
  ...
  ret = cls_cxx_map_get_vals(hctx, new_start, filter_prefix, num_entries - pkeys->size(), &new_keys, ptruncated);
  if (ret < 0)
    return ret;
...
int rgw_bucket_list(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  do {
    rc = get_obj_vals(hctx, start_key, op.filter_prefix, left_to_read, &keys, &truncated);
    if (rc < 0)
      return rc;
  ...
  ret.is_truncated = truncated && !done;
...
#define CHECK_CHUNK_SIZE 1000
  ...
  do {
    rc = get_obj_vals(hctx, start_obj, filter_prefix, CHECK_CHUNK_SIZE, &keys, &truncated);
    if (rc < 0)
      return rc;
...
public:
  ...
  int find_next_key(cls_rgw_obj_key *next_key, bool *found) {
    ...
    string filter = key.name; /* list key starts with key name, filter it to avoid a case where we cross to
                                 different namespace */
    int ret = cls_cxx_map_get_vals(hctx, list_idx, filter, 1, &keys, &truncated);
    if (ret < 0) {
      return ret;
    }

    if (truncated) {
      /* wth? requested for a single entry, should never ever be truncated */
      return -EIO;
    }
...
static int list_plain_entries(cls_method_context_t hctx, const string& name, const string& marker, uint32_t max,
                              list<rgw_cls_bi_entry> *entries, bool *ptruncated)
{
  string filter = name;
  ...
  int count = 0;
  map<string, bufferlist> keys;
  int ret = cls_cxx_map_get_vals(hctx, start_key, filter, max, &keys, ptruncated);
  if (ret < 0) {
    return ret;
  }
                                 ...
                                 list<rgw_cls_bi_entry> *entries, bool *ptruncated)
{
  ...
  if (max > 0) {
    ret = cls_cxx_map_get_vals(hctx, start_key, string(), max, &keys, ptruncated);
...
#define MAX_BI_LIST_ENTRIES 1000
  ...
  bool truncated;
  int ret = list_plain_entries(hctx, op.name, op.marker, max, &op_ret.entries, &truncated); 
  if (ret < 0) {
  ...
  if (!truncated) {
    ret = list_instance_entries(hctx, op.name, op.marker, max - count, &op_ret.entries, &truncated);
    if (ret < 0) {
  ...
  if (!truncated) {
    ret = list_olh_entries(hctx, op.name, op.marker, max - count, &op_ret.entries, &truncated);
    if (ret < 0) {
  ...
  op_ret.is_truncated = (count >= max) || truncated;
  while (count >= max) {
...
#define MAX_USAGE_TRIM_ENTRIES 128
  ret = usage_iterate_range(hctx, op.start_epoch, op.end_epoch, op.user, iter, MAX_USAGE_TRIM_ENTRIES, &truncated, usage_log_trim_cb, NULL);
  if (ret < 0)
    return ret;
...
static int rgw_cls_lc_get_next_entry(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  string filter_prefix;
  bool truncated;
  int ret = cls_cxx_map_get_vals(hctx, op.marker, filter_prefix, 1, &vals, &truncated);
  if (ret < 0)
    return ret;
  if (truncated) {
    /* should never happen */
